...
def hill_sided(_x):
    ...
    alpha_x = scipy.special.expit(blend_rate*(x-center_x))
    alpha_y = scipy.special.expit(blend_rate*(y-center_y))

    # h1 = np.zeros(len(_x))
    # h2 = np.ones(len(_x))
    h1 = amplitude_1 * np.exp(- ((x - center_x)**2 + (y-center_y)**2)/sigma_1) + eps*(x+y)
    h2 = amplitude_2 * np.exp(- ((x - center_x)**2 + (y-center_y)**2)/sigma_2) + eps*(x+y)
