...
export function ResampleView(contianer_, state_) {
    ...
    function submit_params(parameters) {
        ...
        service.submit_recompute({
            ...
            version: msc.shared.version,
            new_version: msc.shared.version + '.1',
            params: parse_param(parameters)
        })
    }


    function parse_param(parameters) {
        let PARAMS = ['k', 'b', 'G', 'g', 'n'];
        let arrayOfStrings = parameters.split(' ');
        let parsed_params = {};
        let i = 0;
        while (i < arrayOfStrings.length) {
            let cur_arg = arrayOfStrings[i]
            if (cur_arg.length === 1 && PARAMS.indexOf(cur_arg) > -1) {
                let cur_val = '';
                for (let j = i + 1; j < arrayOfStrings.length; j++) {
                    let temp_val = arrayOfStrings[j];

                    if (temp_val.length === 1 && PARAMS.indexOf(temp_val) > -1) {
                        break;
                    }
                    else {
                        i = j + 1;
                        cur_val = (cur_val.length === 0) ? temp_val : cur_val.concat(' ' + temp_val);
                    }
                }
